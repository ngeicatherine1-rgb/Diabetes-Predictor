...
def main():
    ...
    try:
        ...
        if len(input_values) != len(FEATURE_NAMES):
            print(
                f"Error: Expected {len(FEATURE_NAMES)} features, "
        ...
        print("=" * 50)
        print(
            f"Input: {', '.join(f'{name}={val}' for name, val in zip(FEATURE_NAMES, input_values))}"
        )
        print("-" * 50)
        print(
            f"Prediction: {'POSITIVE (Diabetes)' if result['prediction'] == 1 else 'NEGATIVE (No Diabetes)'}"
        )
